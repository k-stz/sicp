...
;; will depending on its input retrieve the closed over data. We say the data
;; has a /procedural representation/.

;; Message passing,
;; furthermore the style employed here is called /message passing/ this means we
;; pass some information to an object or process, here the m-cons returned procedure
;; (a function object), and the object decides what sub-routine to run.
;; This is to be contrasted by simply calling the routine directly. Basically whenever
...
;; Which means we need to store 2^a, 3^a and the corresponding logarithm bases
...
;; This means that (* 2^a 3^b) yields a unique numbers. Hence we know that if we
...
(defun remainder-zero-n-times (dividend divisor)
  "Returns the number of times the divisor can be divided by the dividend while at
